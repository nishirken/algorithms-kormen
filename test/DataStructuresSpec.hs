...
dataStructuresSpec = describe "DataStructuresSpec" $ do
  ...
  context "BinaryTree" $ do
    ...
    it "find common parent 1" $ B.findCommonParent 1 2 (B.Node Nothing 1 "1" Nothing) `shouldBe` Nothing
    it "find common parent 2" $ B.findCommonParent 2 7 tree `shouldBe` Just 6
    it "find common parent 3" $ B.findCommonParent 6 5 tree `shouldBe` Just 6
    it "find common parent 4" $ B.findCommonParent 5 2 tree `shouldBe` Just 6
    it "find common parent 5" $ B.findCommonParent 5 8 tree `shouldBe` Just 6
    it "find common parent 6" $ B.findCommonParent 7 8 tree `shouldBe` Just 6
