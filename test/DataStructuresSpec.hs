...
dataStructuresSpec = describe "DataStructuresSpec" $ do
  ...
  context "BinaryTree" $ do
    ...
    it "find common parent 2" $ B.findCommonParent 2 7 tree `shouldBe` (Just (7, "7"))
    it "find common parent 3" $ B.findCommonParent 6 5 tree `shouldBe` Nothing
    it "find common parent 4" $ B.findCommonParent 5 2 tree `shouldBe` (Just (6, "6"))
